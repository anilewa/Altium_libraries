...
clock_t posix_clock_get(posix_clock_t *c, int kind)
{
        ...
        /* at thread level */
        current = clock();
        if (current > _posix_current_thread->btime)
        {
                delta = current - _posix_current_thread->btime;
                _posix_current_thread->btime = current;
                posix_clock_update(&_posix_current_thread->clock, delta);
...
bool posix_free_timer(timer_t timer)
{
        ...
        if (timer >= 0 && timer < TIMER_MAX)
        {
                ...
                {
                        memset(&_posix_timer_table[timer], 0, sizeof(_posix_timer_table[timer]));
                        _posix_timer_table[timer].handler = posix_timer_handler;
                        ret = true;
                }
        }
        return ret;
                                ...
                                struct timespec *tmspec)
{
        ...
        nsecs = res * ticks;
        tmspec->tv_sec = (time_t)(nsecs / 1000000000ll);
...
void posix_signals_thread_code(void)
{
        ...
        while (1)
        {
                ...
                /* reset after waiting so we do not miss anything */
