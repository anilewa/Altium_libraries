...
int fat_mkdir( fs_fcb_t * restrict fcb, fatfs_dirent_t * dirent )
{
    ...
    // have a dotdot-entry pointing to our parent or 0 if pointing to root
    ++direntry;
    if (dirent->dirnode == dirent->fs->rootdir_node)
    {
        dir_set_cluster( direntry, 0 );
    }
    else
...
static int readwrite( fs_fcb_t * restrict fcb, void *buffer, uint32_t size, uint32_t count, uint8_t writemode )
{
    ...
    if ( fcb->u.fat.cluster )
    {
        if ( !fcb->u.fat.pos_cluster )
        {
            ...
            // special case: check if we need to allocate more space; this is when we are
            // exactly at the end of the last allocated cluster
            if (fcb->u.fat.pos == cluster_endpos)
            {
