...
static int fatfs_statvfs( mount_t * restrict mount, struct statvfs *buf );
...
static int findnode( fatfs_t * fs, uint32_t node, fatfs_dirent_t * dirent, const char **name_p )
{
    ...
    while ( *name )
    {
        ...
        else
        {
            ...
            if (dirent->node == 0)
            {
                /* 0 'magic' marker for root dir */
                dirent->node = dirent->fs->rootdir_node;
            }
...
static int fatfs_statvfs( mount_t * restrict mount, struct statvfs *buf)
{
    ...
    /* count free/used blocks in fatdirtable */

    buf->f_bsize    = fatfs->sectorsize;                                   /* File system block size.      */
    buf->f_frsize   = fatfs->sectorsize * fatfs->sectors_per_cluster;     /* Fundamental file system block size.  */
    buf->f_blocks   = fatfs->max_cluster;                                 /* Total number of blocks on file system in units of f_frsize.  */
    buf->f_bfree    = cluster_free_count(fatfs);                  /* Total number of free blocks.                                 */
    buf->f_bavail   = buf->f_bfree;                                       /* Number of free blocks available to non-privileged process.   */
// ???    unsigned long   f_fsid;         /* File system ID.                                              */
    if (mount->flags & MOUNT_FLAG_RDONLY)
    {
