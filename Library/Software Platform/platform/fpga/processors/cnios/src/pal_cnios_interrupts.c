...
/*
 * Interrupt handler location and operation.
 * Normal:
 *      Exception handler is at 'Exception Address'.
 * Jumped:
 *      At 'Exception Address' there is a jump to the exception handler,
 *      exception handler is at 'Jump Address'.
 * Jump only:
 *      Only the jump to the exception handler is in the program (code
 *      containing the exception handler will be loaded later).
 * Handler only:
 *      Only the exception handler is in the code, the jump to the handler from
 *      the exception address is handled by code that's not part of the program.
 */
#if (INTERRUPTS_INTRHANDLERMODE == INTERRUPTS_INTRHANDLERMODE_NORMAL)
#define EXCEPTION_ROUTINE_ADDRESS       INTERRUPTS_EXCEPTION_ADDRESS
#define NO_JUMP                         1
#define NO_ROUTINE                      0
#endif
#if (INTERRUPTS_INTRHANDLERMODE == INTERRUPTS_INTRHANDLERMODE_JUMPED)
#define EXCEPTION_ROUTINE_ADDRESS       INTERRUPTS_EXCEPTION_HANDLER_ADDRESS
#define EXCEPTION_JUMPCODE_ADDRESS      INTERRUPTS_EXCEPTION_ADDRESS
#define NO_JUMP                         0
#define NO_ROUTINE                      0
#endif
#if (INTERRUPTS_INTRHANDLERMODE == INTERRUPTS_INTRHANDLERMODE_JUMPED_JUMP_ONLY)
#define EXCEPTION_ROUTINE_ADDRESS       INTERRUPTS_EXCEPTION_HANDLER_ADDRESS
#define EXCEPTION_JUMPCODE_ADDRESS      INTERRUPTS_EXCEPTION_ADDRESS
#define NO_JUMP                         0
#define NO_ROUTINE                      1
#endif
#if (INTERRUPTS_INTRHANDLERMODE == INTERRUPTS_INTRHANDLERMODE_JUMPED_DESTINATION_ONLY)
#define EXCEPTION_ROUTINE_ADDRESS       INTERRUPTS_EXCEPTION_HANDLER_ADDRESS
#define NO_JUMP                         1
#define NO_ROUTINE                      0
#endif
...
static inline void nios_posix_interrupt_stack_swap(void)
{
...
static inline void nios_posix_interrupt_stack_restore(void)
{
...
static inline void nios_posix_interrupt_save_ctx(void)
{
...
static inline void nios_posix_interrupt_restore_ctx(void)
{
...
static inline void nios_common_interrupt_handler(uint32_t number)
{
    ...
    #if ( __TIMERS__ > 0 )
    if (number == PAL_ARCH_CLOCK_INTNO )
    {
...
static inline void nios_common_interrupt_handler(uint32_t number)
{
    ...
    else if (number == PAL_ARCH_CLOCK_INTNO )
    {
...
/* disable warning 752 interrupt function "..." does not save register ".." */
#pragma warning 752
#if (!NO_ROUTINE)
__interrupt() __FRAME() void nios_external_interrupt(void)  __at(EXCEPTION_ROUTINE_ADDRESS)
{

    INTERRUPT_SAVE_CTX()

    mask = nios_get_pending_interrupts();
    mask &= -(int32_t)mask;     // handle only highest interrupt
    ...
    INTERRUPT_NIOS_HANDLER(10)
    #endif
    #if GENERATE_NIOS_INTERRUPT_HANDLER( 11 )
    ...
    INTERRUPT_NIOS_HANDLER(22)
    #endif
    #if GENERATE_NIOS_INTERRUPT_HANDLER( 23 )
    ...
    INTERRUPT_NIOS_HANDLER(26)
    #endif
    #if GENERATE_NIOS_INTERRUPT_HANDLER( 27 )
...
#if (!NO_JUMP)
__interrupt() __frame() void nios_external_interrupt_jump(void) __at(EXCEPTION_JUMPCODE_ADDRESS)
{
#if 0
    __asm("orhi    et, r0, @hi(%0)" : : "i"(EXCEPTION_ROUTINE_ADDRESS));
    __asm("ori     et, et, @lo(%0)" : : "i"(EXCEPTION_ROUTINE_ADDRESS));
#else
    __asm("orhi    et, r0, %0" : : "i"(((EXCEPTION_ROUTINE_ADDRESS) >> 16) & 0xffff));
    __asm("ori     et, et, %0" : : "i"((EXCEPTION_ROUTINE_ADDRESS) & 0xffff));
#endif
    __asm("jmp     et");
 }
#endif /* (!NO_JUMP) */
